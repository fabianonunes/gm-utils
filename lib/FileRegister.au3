...
Func FileRegister($ext, $cmd, $verb, $def = 0, $icon = "", $desc = "")
    Local $loc = RegRead("HKCR\." & $ext, "")
    If @error Then
        RegWrite("HKCR\." & $ext, "", "REG_SZ", $ext & "file")
        $loc = $ext & "file"
    EndIf
    Local $curdesc = RegRead("HKCR\" & $loc, "")
    If @error Then
        If $desc <> "" Then
    ...
    Else
        ...
        EndIf
    EndIf
    Local $curverb = RegRead("HKCR\" & $loc & "\shell", "")
    If @error Then
        If $def = 1 Then
    ...
    Else
        ...
        EndIf
    EndIf
    Local $curcmd = RegRead("HKCR\" & $loc & "\shell\" & $verb & "\command", "")
    If Not @error Then
    ...
    If $icon <> "" Then
        Local $curicon = RegRead("HKCR\" & $loc & "\DefaultIcon", "")
        If @error Then
...
Func FileUnRegister($ext, $verb)
    ...
    If Not @error Then
        ...
        EndIf
        Local $oldverb = RegRead("HKCR\" & $loc & "\shell", "oldverb")
        If Not @error Then
            RegWrite("HKCR\" & $loc & "\shell", "", "REG_SZ", $oldverb)
        Else
            RegDelete("HKCR\" & $loc & "\shell", "")
        EndIf
        Local $olddesc = RegRead("HKCR\" & $loc, "olddesc")
        If Not @error Then
            RegWrite("HKCR\" & $loc, "", "REG_SZ", $olddesc)
        Else
            RegDelete("HKCR\" & $loc, "")
        EndIf
        Local $oldcmd = RegRead("HKCR\" & $loc & "\shell\" & $verb & "\command", "oldcmd")
        If Not @error Then
